...
async function handlePersonaChoice(openid, msgId, choice, sendFn) {
  ...
  if (choice === '8') {
    ...
    await sendFn(openid, msgId, '发一张照片给我，或者直接描述ta的外貌和名字都可以～\n（照片可选，文字描述也行）');
    return true;
  }
...
async function handleCustomImage(openid, msgId, input, sendFn) {
  // input may be an image URL (from QQ attachment parsing) or plain text description
  const isUrl = input.startsWith('http');
  const persona = isUrl
    ? { name: 'ta', personality: '神秘而温柔，性格随对话自然演化', refImageUrl: input, refDescription: '按你上传的照片外貌' }
    : { ...parseCustomDesc(input), refImageUrl: '' };
  setPersona(openid, persona);
  setSetupStep(openid, 'choose_relationship');
  await sendFn(openid, msgId, `好的！我已经记住了～\n\n${RELATIONSHIP_PROMPT}`);
  return true;
